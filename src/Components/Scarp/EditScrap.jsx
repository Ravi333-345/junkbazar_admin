...
const EditScrap = () => {
  ...
  const scrapDetails = location.state ? location.state.item : null;
  ...
  useEffect(() => {
    setPreview(scrapDetails.docUrl);
    if (imageKey === "") {
      setImageUrl(scrapDetails.docPath);
    } else setImageUrl(imageKey);
  }, [imageKey]);

  const [formValues, setFormValues] = useState({
    scrapName: scrapDetails.scrapName,
    price: scrapDetails.price,
    quantityType: scrapDetails.quantityType,
  ...
  const uploadData = async () => {
    if (
      ...
      formValues.scrapName != ""
    ) {
      const dataPayload = {
        scrapName: formValues.scrapName,
        price: parseInt(formValues.price),
        quantityType: quantityType ? quantityType : scrapDetails?.quantityType,
        imageKey: imageUrl,
        scrapId: scrapDetails.scrapId,
      };

      console.log("dataPayload", dataPayload);

      try {
        const resp = await axiosInstance.post("/editScrap", dataPayload);
